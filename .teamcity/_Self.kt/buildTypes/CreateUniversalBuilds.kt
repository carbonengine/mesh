...
class UniversalBuild() : BuildType({
    ...
    params{
        ...
        text("file-matchers",
            "-m '*.so:%system.teamcity.build.workingDir%/lib' -m '*.dylib:%system.teamcity.build.workingDir%/lib' -m '*.a:%system.teamcity.build.workingDir%/lib' -m 'bin/**/cmfprocessor*:%system.teamcity.build.workingDir%/bin' -m 'CarbonMeshViewer*:%system.teamcity.build.workingDir%/bin'",
