	...
	for( uint32_t meshIndex = 0; meshIndex < content->m_cmfData->meshes.size(); meshIndex++ )
	{
		...
		for( const MeshLod& lod : mesh.lods )
		{
			...
			for( int attributeIndex = 0; attributeIndex < mesh.decl.size(); attributeIndex++ )
			{
				...
				//{
				//	const LodMeshArea& area = lod.areas[areaIndex];
					...
					if( lod.ib.size > 0 )
					{
						...
						//const uint8_t* indexData = content->m_fileContent.data() + indexBufferOffset + area.firstElement * 3 * lod.ib.stride;
                        ...
                        */
						...
						for( uint32_t i = 0; i < numIndices; i++ )
						{
							...
							if( lod.ib.stride == 2 )
							{
								index = *reinterpret_cast<const uint16_t*>( indexData );
							}
							else
							{
								index = *reinterpret_cast<const uint32_t*>( indexData );
							}

							Vector4 value = readAttribute( attribute, vertexData + index * lod.vb.stride );
							...
							indexData += lod.ib.stride;
						}
					}
					else
					{
						uint32_t vertexBufferOffset = content->m_cmfHeader->sections[lod.vb.index].offset + lod.vb.offset;
						//const uint8_t* vertexData = content->m_fileContent.data() + vertexBufferOffset + area.firstElement * 3 * lod.vb.stride;
						const uint8_t* vertexData = content->m_fileContent.data() + vertexBufferOffset;
						...
						{
							Vector4 value = readAttribute( attribute, vertexData );
...
int main()
{
...

	{
        ...
        std::vector<uint8_t> fileData = optimizer.toCmf();
