...
def parse_vertex_declarations(shader_path):
    ...
    with open(shader_path, "r") as vertex_shader:
        for line in vertex_shader:
            ...
            print("match in file '%s': %s" % (os.path.basename(shader_path), match.groups()))
    ...
    print("Parsed vertex declarations for shader '%s': %s" % (os.path.basename(shader_path), declarations))
    return declarations
