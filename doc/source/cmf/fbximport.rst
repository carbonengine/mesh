...
* ``cmf_path`` - path to the output CMF file (the file will be created or ovewritten)
* ``options`` - various options to control the behavior of the importer, see below

The program will return 0 (via errorcode) on successful import and non-zero code if an error occured. In case of an error, the error message will be printed to the error stream.

Options for the progam are:
...
* ``--path source_path`` - overrides the source FBX path included in the CMF metadata. See `Metadata <Metadata_>`_ below.
* ``--version`` or ``-v`` - print out the importer version and exit. Note that the importer version is not the same as carbon-mesh component version nor the CMF format version.
...
``mesh.filter``
  Type ``array`` of ``string``, default ``[]``. Names of meshes to include in the CMF file. This setting allows to only import meshes with specific names. When the array is empty, all meshes are imported.
...
``mesh.tangents``
  Type ``number``, default ``1``. Number of tangent spaces (tangent and binormal vectors) to generate/import. The importer would only try to import tangents for UV set 0, any extra tangents spaces are always generated. If the
...
``mesh.alwaysComputeTangents``
  Type ``boolean``, default ``true``. Ignore tangent data in the source FBX file, i.e. always recompute tangents for UV set 0. This setting only has effect when ``mesh.tangents`` is greater than zero.
...
``mesh.bonesPerVertex``
  Type ``number``, default ``4``. Maximum number of bones affecting a mesh vertex. If the source vertex is affected by more that the specified number of bones, only the maximum specified number of bones with largest weights
...
``mesh.regenerateNormals``
  ...
  on import, see `Morph targets <Morph targets>_`_.
...
``mesh.morphTargets.useCustomNormals``
  Type ``boolean``, default ``false``. Use per-blend shape normal data stored in custom FBX properties instead of regenerating blend shape normals on import. See `Morph targets <Morph targets>_`_. This setting only has effect if 
...
``skeleton.filter``
  Type ``array`` of ``string``, default ``[]``. Names of skeletons (root bone nodes) to include in the CMF file. This setting allows to only import skeletons with specific names. When the array is empty, all skeletons are imported.
...
``animation.filter``
  Type ``array`` of ``string``, default ``[]``. Names of animation tracks to include in the CMF file. This setting allows to only import animations with specific names. When the array is empty, all animation tracks are imported.
...
* The importer uses a legacy coordinate system transformation from FBX coordinate space to CMF space. It may not always produce expected results, but it is kept for backwards compatibility. If there are issues with 
...
The importer suppors importing morph targets from FBX blend shapes. The importer will always store vertex positions in morph target buffer. It will also store normals and/or tangent spaces according to ``mesh.normals`` and ``mesh.tangents``
...
base mesh normals would match blend shape normals for parts of the mesh not affected by the blend shape.
...
for the mesh containing a blend shape. The name of the attribute should match the name of the blend shape with ``bsNormals_`` prepended to it. The attribute must be a string containing a base64-encoded array of per face vertex (not per vertex!) normals vectors 
