...
void UIRenderer::MeshDetailsWindow( AppState& appState )
{
	...
	if( ImGui::BeginTabBar( "##viewtabs" ) )
	{
		...
		{
			RenderHierarchyTab( cmfContent, mesh );
			ImGui::EndTabItem();
		}
...
void UIRenderer::RenderHierarchyTab( CmfContent* cmfContent, const cmf::Mesh& mesh )
{
	...
	if( !ImGui::TreeNodeEx( meshHeader.c_str(), ImGuiTreeNodeFlags_DefaultOpen ) )
	{
		ImGui::EndChild();
		return;
	}
	...
	{
		const auto& skeleton = skeletons[mesh.skeleton];
		std::string skelHeader = "Skeleton: " + cmf::ToStdString( skeleton.name ) +
		...
		if( ImGui::TreeNodeEx( skelHeader.c_str(), ImGuiTreeNodeFlags_DefaultOpen ) )
		{
			...
			std::function<void( int )> renderBone = [&]( int bi )
			{
		...
		while( clipper.Step() )
		{
			for( int i = clipper.DisplayStart; i < clipper.DisplayEnd; ++i )
			{
				...
				for( int col = 0; col < (int)activeCurveCols.size(); ++col )
				{
					...
					{
					case CurveColumn::Interpolation: ImGui::TextUnformatted( interp );        break;
					case CurveColumn::KnotType:      ImGui::TextUnformatted( knotTypeName );  break;
					case CurveColumn::ValueType:     ImGui::TextUnformatted( valueTypeName ); break;
					case CurveColumn::Dimensions:    ImGui::Text( "%u", (uint32_t)curve.valueDimension ); break;
					case CurveColumn::Knots:         ImGui::Text( "%u", curve.knotCount );    break;
					}
				}
			}
		}
